...
int main() {
    ...
    std::vector<TestV> v_tests{
        ...
        {EXPR(q + q), {0, 2}}, {EXPR(r + q), t},
        {EXPR(r += r), {2, 0}}, {EXPR(q += q), {0, 2}}, {EXPR(r), {2, 0}},
        {EXPR(q), {0, 2}}, {EXPR(r - s), {1, 0}}, {EXPR(q - p), {0, 1}},
        {EXPR(r -= s), {1, 0}}, {EXPR(q -= p), {0, 1}}, {EXPR(r), {1, 0}},
        {EXPR(q), {0, 1}}, {EXPR(r * 5), {5, 0}}, {EXPR(q * 5), {0, 5}},
        {EXPR(5 * r), r * 5}, {EXPR(5 * q), q * 5}, {EXPR(r = s), s},
        {EXPR(q = p), p}, {EXPR(r *= 5), s * 5}, {EXPR(q *= 5), p * 5},
        {EXPR(r), s * 5}, {EXPR(q), p * 5}
    };
    ...
    for (const auto& [expr, res, ans] : v_tests) {
        ...
        std::cerr << std::setw(25) << std::left << expr << " = {" << res.x
                    << ", " << res.y << "}"
